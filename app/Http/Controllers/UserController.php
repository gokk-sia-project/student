...
$this->request = $request;
    ...
    public function getUsers(){ //LISTUSER - list all user data
    ...
    public function getID($id){ //GETID - get using with id
        ...
        {
            return $this->errorResponse('Student ID Does Not Exist', Response::HTTP_NOT_FOUND);
        }
    }
    ...
    public function deleteUser($id){ //DELETUSER - delete existing user
        $user = User::where('student_id', $id)->delete();
        if($user){
            return $this->successResponse($user);
        }
        {
